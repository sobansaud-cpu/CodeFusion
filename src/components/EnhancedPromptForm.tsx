...
export const EnhancedPromptForm: React.FC<EnhancedPromptFormProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (aiModel === 'openrouter') {
      if (!openRouterModel.trim()) {
        toast.error('Please enter or select an OpenRouter model');
        return;
      }
    ...
    if (aiModel === 'gemini') {
      if (!geminiModel.trim()) {
        toast.error('Please enter or select a Gemini model');
        return;
      }
      if (!customApiKey.trim()) {
    ...
    if (aiModel === 'gemini') {
      options.openRouterModel = geminiModel;
    }
  ...
  // The OpenRouter model dropdown input allowing free text input
  // This is done by using Select with creatable behavior simulated by allowing a text input as well.
  ...
  return (
    <div className="space-y-6">
            ...
            {showAdvancedOptions && (
              <div className="space-y-4 pl-6 border-l-2 border-gray-200">
                ...
                {aiModel === 'openrouter' && (
                  <>
                    <div className="space-y-2">
                      <Label>OpenRouter Model (select or enter your custom model)</Label>
                      <input
                        type="text"
                        placeholder="Type or paste OpenRouter model here..."
                        className="w-full rounded border border-gray-300 px-3 py-2 text-sm"
                        value={openRouterModel}
                        onChange={(e) => setOpenRouterModel(e.target.value)}
                      />
                      <p className="text-xs text-gray-500">
                        Enter your OpenRouter model ID or paste from your models list.
                      </p>
                    </div>
                    <div className="space-y-2">
                      <Label>OpenRouter API Key</Label>
                ...
                {aiModel === 'gemini' && (
                  <>
                    {/* <div className="space-y-2">
                      ...
                      <input
                        type="text"
                        placeholder="Type or paste Gemini model here..."
                        className="w-full rounded border border-gray-300 px-3 py-2 text-sm"
                        value={geminiModel}
                ...
                {(aiModel !== 'openrouter' && aiModel !== 'gemini' && (aiModel === 'gpt-4o' || aiModel === 'gpt-4' || 
                  aiModel === 'claude-3-opus' || aiModel === 'claude-3-sonnet')) && (
                  <div className="space-y-2">
                    <Label>API Key (Optional)</Label>
                    <Input
      ...
      <Button
        ...
        onClick={handleSubmit}
        disabled={loading || remainingGenerations === 0 || !prompt.trim() ||
          (aiModel === 'openrouter' && (!openRouterModel.trim() || !customApiKey.trim())) ||
          (aiModel === 'gemini' && (!geminiModel.trim() || !customApiKey.trim()))
        }
        className="w-full h-12 text-lg font-semibold"
        size="lg"
      >
        ...
        <Card className="border-blue-200 bg-blue-50 dark:bg-blue-900/20">
          <CardContent className="pt-6">
            ...
            <div className="space-y-2 text-sm">
              ...
              <p><strong>AI Model:</strong> {AI_MODELS.find(m => m.value === aiModel)?.label}</p>
              {(aiModel === 'openrouter') && <p><strong>OpenRouter Model:</strong> {openRouterModel}</p>}
              {(aiModel === 'gemini') && <p><strong>Gemini Model:</strong> {geminiModel}</p>}
              {selectedImage && <p><strong>Image:</strong> Attached for analysis</p>}
            </div>
          </CardContent>
        </Card>
